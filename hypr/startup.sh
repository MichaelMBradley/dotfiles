...
if [ "$(uname -n)" = "chonk" ]; then
  ...
  hyprctl dispatch -- exec kitty --listen-on "unix:/tmp/kitty-command-1"
  sleep 0.5
  hyprctl dispatch -- exec kitty --listen-on "unix:/tmp/kitty-command-2"
  ydotool mousemove -x 0 -y 90
  sleep 0.5
  hyprctl dispatch -- exec kitty --listen-on "unix:/tmp/kitty-command-3"
  sleep 0.5
